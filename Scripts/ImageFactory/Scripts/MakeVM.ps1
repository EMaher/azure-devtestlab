...
function makeUpdatedTemplateFile ($origTemplateFile, $outputFile) {
    ...
    #get OsType so we can add the correct sysprep or deprovosion artifact.  First check for gallery images
    $osType = $armTemplate.resources[0].properties.galleryImageReference.osType
    if ($osType -eq $null) {
        $dtlCustomImageResourceId = $armTemplate.resources[0].properties.customImageId
        $dtlCustomImageResource = Get-AzureRmResource -ResourceId $dtlCustomImageResourceId -ApiVersion '2017-04-26-preview'
        #custom image was specified, need to get the os type from the image properties
        if ($dtlCustomImageResource.properties.vm.windowsOsInfo -ne $null) {
            $osType = 'Windows'
        }
        elseif ($dtlCustomImageResource.properties.vm.linuxOfInfo -ne $null) {
            $osType = 'Linux'
        }
    }
    ...
    switch ($osType) {
        'Windows' { $artifactName = 'windows-sysprep'}
        'Linux' { $artifactName = 'linux-deprovision'}
        default { Write-Error "##[error]Unable to determine OS type for resource.  Machine will not be sysprepped/deprovisioned" }
    }
    $newArtifact = @{}
